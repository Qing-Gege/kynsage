...
给**不写代码的人**用的 Claude Code / Grok 指挥台——
...
1. 点上面的按钮，到下载页面，下载 **`kynsage-x.x.x-win-x64.exe`** 这个文件（[也可以点这里直接下最新版](https://github.com/Qing-Gege/kynsage/releases/latest/download/kynsage-0.0.12-win-x64.exe)）
...
**🍎 用 Mac 的朋友**（Apple 芯片 / M 系列）：下载 **`kynsage-0.0.12-mac-arm64.dmg`**（[点这里直接下](https://github.com/Qing-Gege/kynsage/releases/latest/download/kynsage-0.0.12-mac-arm64.dmg)），拖进「应用程序」即可。
...
- **Claude Code / Grok 随时切换**——在设置里选定 AI 引擎，新建、历史项目、恢复、状态提醒和自动标题会整套切换；默认仍是 Claude Code，已经打开的对话不受设置变化影响。
- **一键起同事，自动进 Agent**——点「新建同事」，就在当前文件夹起一个工作台并自动拉起所选 Agent，全程你不用敲一条命令。
...
- **要正经整理时能摊开**——窄栏够瞟一眼，不够动手。点「展开」摊成完整文件管理器，见下节。

### ↔️ 要正经整理文件时，一键把文件区摊开

平时文件区是窄窄一条，够你瞟一眼 AI 动了哪些文件。可真要**动手整理**时——按时间翻一遍最近改了什么、挑出几个大文件、把一堆文件归到不同文件夹里——那条窄栏就不够用了。

点文件区右下角的「**展开**」，它会**横向摊开吃掉终端那一半**，变成一个正经的文件管理器：

- **四列表格视图**——名称、修改时间、类型、大小。**点列头就排序**，再点一次反向。找「昨天改的那个文件」直接按修改时间倒排，不用一个个看。
- **展开了才真的多东西**——不是把窄栏拉宽留一片空白，而是补齐窄态放不下的三列信息。窄态挤在名字下一行的时间和大小，到这儿各自成列，对得整整齐齐。
- **修改时间给绝对时间**——窄态显示「3 分钟前」这种相对时间，展开态改成完整日期时间。多列视图是用来**比对和排序**的，「3 分钟前」和「5 分钟前」谁先谁后，读起来费劲。
- **侧栏还在**——吃的只是终端那一半，同事列表和标签页都留着，你能一边整理文件一边看谁在忙。
- **随时退回**——再点一下「收起」，或者直接按 **`Esc`**。在输入框和编辑器里按 Esc 不会误触发（那归重命名取消用）。
- **记得你的选择**——展开着关掉应用，下次打开还是展开的。
- **底部状态条报数**——「共 N 项」，多选时还告诉你「已选 N 项」和这些文件**加起来多大**。清空间前先估一估，不用自己加。
- **找不到按钮也能进**——在文件区空白处右键，菜单里同样有「展开为完整文件操作区」。

> 一个贴心处理：展开着的时候点「新建同事」，文件区会**自动收回**。因为启动页在终端那半，展开态会把它整个盖住，不收回你会以为点了没反应。
...
- **历史对话一键翻**——「历史对话」下拉列出这个文件夹里所有聊过的 Claude 会话（带标题和时间），点一条就恢复；不想留的把鼠标移上去点 `×`，二次确认后即可删除。
