...
## 2026-08-14 — 品牌设置移除与 macOS 打包修复（v0.0.14）

- 移除「设置 → 协作」中已经失效的品牌主标题、副标题配置。
- 产品品牌统一固定，旧版本的品牌字段会在升级时自动清理。
- 同步更新 Windows 与 macOS 安装包版本号。
- macOS 包固定执行 ad-hoc 完整签名并在构建后严格校验，避免 Gatekeeper 误报应用“已损坏”。
